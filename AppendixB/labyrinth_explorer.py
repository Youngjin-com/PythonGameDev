...
def draw_3d_space(sx, sy, sa): # 3차원 공간 계산
    cvs.create_rectangle(480, 0, 1200, 300, fill="navy", outline="")
    for i in range(20): # 바닥 표시
...
def main(): # 메인 처리
    ...
    if MAZE[int(pl_y/SIZE)][int(pl_x/SIZE)]==1:
        cvs.create_text(WIDTH/2, HEIGHT/2, text="축하합니다!\n골에 도착했습니다.", font=FNT, fill="white")
        return
    root.after(50, main)

root = tkinter.Tk()
root.title("3D 던전 라비린스 익스플로러")
root.bind("<Key>", pkey)
