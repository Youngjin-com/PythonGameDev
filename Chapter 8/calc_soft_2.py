...
def btn_on(typ):
    try:
        ...
        if typ=="/":
            if v2==0:
                tkinter.messagebox.showinfo("","0으로 나눌 수 없습니다")
                return
            ans = str(v1/v2)
        lbl["text"] = "= "+ans
    except:
        tkinter.messagebox.showinfo("","엔트리에 숫자를 입력해주세요")

def btn_add(): btn_on("+") # + 버튼을 눌렀을 때 불러올 함수
def btn_sub(): btn_on("-") # －버튼을 눌렀을 때 불러올 함수
def btn_mul(): btn_on("*") # ×버튼을 눌렀을 때 불러올 함수
def btn_div(): btn_on("/") # ÷버튼을 눌렀을 때 불러올 함수

root = tkinter.Tk()
root.geometry("500x200")
root.title("계산기 ver.2")
FNT = ("System", 12)
